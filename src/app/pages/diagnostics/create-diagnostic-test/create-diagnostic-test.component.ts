...
export class CreateDiagnosticTestComponent implements OnInit {
  ...
  getDetails() {
    this.spinner.show()
    this.diagnosticsService.getProductDetails(this.editId).subscribe(res => {
      this.spinner.hide()
      ...
      if (success && data) {
        ...
        this.form.patchValue({ hbDiscount, costPrice, description, metaTitle, metaDescription } as any)
        if (process) {
          process.forEach(el => {
            this.addProcess(el)
          });
  ...
  onSubmit() {
    ...
    if (valid) {
      this.spinner.show()
      this.diagnosticsService.updateProduct(this.editId, value).subscribe(res => {
        this.spinner.hide()
        const { success } = res;
        if (success) {
