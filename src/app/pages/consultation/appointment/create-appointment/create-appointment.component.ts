...
export class CreateAppointmentComponent implements OnInit {
    ...
    this.phoneNumber$.pipe(debounceTime(500)).subscribe(value => {
      if (value) {
        ...
        if (value.length > 4) {
          ...
          this.eCommerceService.getUserList({ userIdentifier: value }).subscribe((res: any) => {
  ...
  bookAppointment() {
    ...
    if (this.form.valid) {
      ...
      this.editId
        ?
        this.apiService.updateAppointment(data)
          .subscribe((res: any) => {
        ...
        :
        this.apiService
          .createAppointment(data).subscribe((res: any) => {
            this.spinner.hide();
  ...
  getSlots(consultantId, date, presistInputValue?: boolean) {
    ...
    this.spinner.show();
    this.consultantService.getSlots(consultantId, date, true).subscribe((res: any) => {
      this.spinner.hide();
      ...
      if (this.editId) {
        if (getFormatedDate(this.apptData.date) == date) {
          let [startTime, endTime] = this.apptData.primaryTimeSlot.split('-').map(el => el.trim())
          this.slots.unshift({
            startTime,
            endTime,
            startTime12: time24to12(startTime),
  ...
  getConsultantDetail() {
    this.spinner.show()
    this.contactsService.getUserDetail(this.user._id).subscribe(res => {
      this.spinner.hide()
      let { success, data } = res
      if (success) {
